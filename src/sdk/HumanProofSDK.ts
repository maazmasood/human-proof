...
import type { HumanAssertion, HumanChallenge } from "../shared/types.js";
...
function saveCredentialId(id: string): void {
  try {
    ...
    console.warn("Failed to save credentialId to localStorage", e);
  }
}
...
export class HumanProofSDK {
  ...
  private listeners: Map<HumanProofEvent, Set<(...args: any[]) => void>> = new Map(); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  async enroll({ userId, displayName }: EnrollOptions): Promise<string> {
    ...
    const optionsUrl = new URL(this.config.enrollPath, this.config.apiBase || location.origin);
    ...
    const options = (await optionsRes.json()) as PublicKeyCredentialCreationOptions & {
      excludeCredentials?: { id: string; type: string }[];
    };

    // 2. Convert challenge + user.id to ArrayBuffer (WebAuthn requires this)
    // Note: options come from JSON, so challenge/user.id are base64url strings at runtime
    const createOptions: PublicKeyCredentialCreationOptions = {
      ...options,
      challenge: base64URLToBuf(options.challenge as unknown as string),
      user: {
        ...options.user,
        id: base64URLToBuf(options.user.id as unknown as string),
      },
    ...
    const completeRes = await fetch(this.config.apiBase + this.config.enrollCompletePath, {
      method: "POST",
  ...
  async prove(action: string): Promise<HumanAssertion> {
    ...
    const challengeRes = await fetch(this.config.apiBase + this.config.challengePath, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async protectedFetch(url: string, options: ProtectedFetchOptions = {}): Promise<Response> {
    ...
    try {
      ...
      if (headers.get("content-type")?.includes("application/json") || typeof body === "string") {
        const parsed = (body ? JSON.parse(body as string) : {}) as Record<string, unknown>;
  ...
  private async parseSpki(spki: ArrayBuffer): Promise<{ jwk: JsonWebKey; alg: number }> {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Unsupported public key algorithm in authenticator", { cause: _e });
      }
    }
  }

  /** Event Emitter interface for Browser */
  public on(event: HumanProofEvent, listener: (...args: any[]) => void): this {
    if (!this.listeners.has(event)) this.listeners.set(event, new Set());
    this.listeners.get(event)!.add(listener);
    return this;
  }

  public off(event: HumanProofEvent, listener: (...args: any[]) => void): this {
